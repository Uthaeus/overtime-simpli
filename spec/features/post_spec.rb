...
describe 'navigate' do
  ...
  describe 'index' do
    ...
    it 'has a scope so that only post creators can see their posts' do 
      post1 = Post.create(date: Date.today, rationale: "asdfadfasdf", user_id: @user.id)
      post2 = Post.create(date: Date.today, rationale: "asdfadfasdf", user_id: @user.id)
      ...
      post_from_other_user = Post.create(date: Date.today, rationale: "unique stuff", user_id: other_user.id)

      visit posts_path

      expect(page).to_not have_content(/unique stuff/)
    end
  end
